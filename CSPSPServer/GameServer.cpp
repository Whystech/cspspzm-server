...
static bool IsRoundBasedMode(int gameType)
{
	return gameType == TEAM || gameType == EXTERMINATION || gameType == INFECTION || gameType == HORDE;
}

int GameServer::LoadServerConfigInt(const char* file, char* key, int defaultValue, int minimum, int maximum)
{
...
void GameServer::Init() {
	...
	char* port = GetConfig("data/config.txt","port");
	if (port != NULL) {
		mPort = abs(atoi(port));
		delete[] port;
	...
	if (file == NULL) {
		cout << "Error: data/guns.txt could not be opened\n";
		mHasError = true;
		return;
	}
	char line[1024]; 
	if (!fgets(line,1024,file) || sscanf(line,"%d",&mNumGuns) != 1 || mNumGuns < 1 || mNumGuns > MAX_GUNS) {
		cout << "Error: Invalid gun count in data/guns.txt\n";
		fclose(file);
		mHasError = true;
		return;
	}
	...
	for (int i=0; i<MAX_GUNS; i++) mGuns[i].mId = -1;
	for (int i=0;i<mNumGuns;i++) {
		if (!fgets(line,1024,file)) {
			cout << "Error: Missing gun rows in data/guns.txt\n";
			fclose(file);
			mHasError = true;
			return;
		}
	...
	if (file != NULL) {
		while (fgets(line,1024,file)) {
			...
			char buffer2[128];
			s = line;
			int n = sscanf(s,"%127s %127s",buffer,buffer2);
			if (n < 1 || buffer[0] == '#') continue;
			...
			if (n == 2) {
				if (stricmp(buffer2,"tdm") == 0) {
					type = TEAM;
				}
				else if (stricmp(buffer2,"ffa") == 0) {
					type = FFA;
				}
				else if (stricmp(buffer2,"ctf") == 0) {
					type = CTF;
				}
				else if (stricmp(buffer2,"extermination") == 0) {
					type = EXTERMINATION;
				}
				else if (stricmp(buffer2,"infection") == 0) {
					type = INFECTION;
				}
				else if (stricmp(buffer2,"horde") == 0) {
					type = HORDE;
				}
				else {
					validType = false;
					cout << "Warning: Ignoring unknown map mode " << buffer2 << " for " << buffer << "\n";
				}
			}
	...
	file = fopen("data/bot_names.txt", "r");
	if (file != NULL) {
		while (fgets(line,1024,file)) {
			...
			*end = '\0';
			...
			name[31] = '\0';
			mBotNames.push_back(name);
		}
		fclose(file);
	}
	if (mBotNames.empty()) {
		static char defaultNames[][32] = {
			"Alpha", "Bravo", "Charlie", "Delta", "Echo", "Foxtrot", "Ghost", "Hunter"
		};
		for (int i=0; i<8; i++) mBotNames.push_back(defaultNames[i]);
	}
	...
	char* friendlyFire = GetConfig("data/config.txt","friendlyfire");
	if (friendlyFire != NULL && stricmp(friendlyFire,"on") == 0) {
		mFriendlyFire = ON;
	}
	else if (friendlyFire != NULL && stricmp(friendlyFire,"off") == 0) {
		mFriendlyFire = OFF;
	}
	else {
		mFriendlyFire = ON;
	...
	char* autoBalance = GetConfig("data/config.txt","autobalance");
	if (autoBalance != NULL && stricmp(autoBalance,"on") == 0) {
		mAutoBalance = ON;
	}
	else if (autoBalance != NULL && stricmp(autoBalance,"off") == 0) {
		mAutoBalance = OFF;
	}
	else {
		mAutoBalance = ON;
	...
	char* maxPlayers = GetConfig("data/config.txt","maxplayers");
	if (maxPlayers != NULL) {
		mNumMaxPlayers = abs(atoi(maxPlayers));
		delete[] maxPlayers;
		if (mNumMaxPlayers < 1) mNumMaxPlayers = 8;
	...
	char* bots = GetConfig("data/config.txt","bots");
	...
	if (mInfectedBotCount > mNumMaxPlayers) mInfectedBotCount = mNumMaxPlayers;
	char* ctBotCount = GetConfig(gameplayConfig,"ct_bot_count");
	mCTBotCount = ctBotCount == NULL ? 3 : abs(atoi(ctBotCount));
	delete[] ctBotCount;
	if (mCTBotCount > mNumMaxPlayers-mInfectedBotCount) {
		mCTBotCount = mNumMaxPlayers-mInfectedBotCount;
	}
	mBotCount = mInfectedBotCount+mCTBotCount;
	mHordeWave = 1;
	char* hordeWaveDelay = GetConfig(gameplayConfig,"horde_wave_delay");
	mHordeWaveDelay = hordeWaveDelay == NULL ? 3 : abs(atoi(hordeWaveDelay));
	...
	delete[] hordeRegroup;
	char* hordeReviveSurvivors = GetConfig(gameplayConfig,"horde_revive_survivors");
	mHordeReviveSurvivors = hordeReviveSurvivors != NULL && stricmp(hordeReviveSurvivors,"on") == 0;
	delete[] hordeReviveSurvivors;
	char* armorDamageReduction = GetConfig(gameplayConfig,"armor_damage_reduction");
	Person::mArmorDamageReduction = armorDamageReduction == NULL ? 30 : atoi(armorDamageReduction);
	delete[] armorDamageReduction;
	if (Person::mArmorDamageReduction < 0) Person::mArmorDamageReduction = 0;
	else if (Person::mArmorDamageReduction > 100) Person::mArmorDamageReduction = 100;
	Person::mCTMaxHealth = LoadServerConfigInt(gameplayConfig,"ct_max_health",100,1,32767);
	Person::mTMaxHealth = LoadServerConfigInt(gameplayConfig,"t_max_health",150,1,32767);
	Person::mCTMaxArmor = LoadServerConfigInt(gameplayConfig,"ct_max_armor",100,0,32767);
	Person::mTMaxArmor = LoadServerConfigInt(gameplayConfig,"t_max_armor",100,0,32767);
	Person::mCTSpawnHealth = LoadServerConfigInt(gameplayConfig,"ct_spawn_health",100,1,Person::mCTMaxHealth);
	Person::mTSpawnHealth = LoadServerConfigInt(gameplayConfig,"t_spawn_health",150,1,Person::mTMaxHealth);
	Person::mCTSpawnArmor = LoadServerConfigInt(gameplayConfig,"ct_spawn_armor",0,0,Person::mCTMaxArmor);
	Person::mTSpawnArmor = LoadServerConfigInt(gameplayConfig,"t_spawn_armor",0,0,Person::mTMaxArmor);
	Person::mStartingMoney = LoadServerConfigInt(gameplayConfig,"starting_money",800,0,32767);
	Person::mPickupRadius = LoadServerConfigFloat(gameplayConfig,"pickup_radius",16.0f,1.0f,256.0f);
	Person::mDamageSpeedMultiplier = LoadServerConfigFloat(gameplayConfig,"damage_speed_multiplier",0.1f,0.0f,1.0f);
	Person::mAcceleration = LoadServerConfigFloat(gameplayConfig,"player_acceleration",0.0005f,0.0f,1.0f);
	Person::mDeceleration = LoadServerConfigFloat(gameplayConfig,"player_deceleration",0.0005f,0.0f,1.0f);
	ServerBot::mDecisionMin = LoadServerConfigInt(gameplayConfig,"bot_decision_min_ms",250,1,60000);
	ServerBot::mDecisionMax = LoadServerConfigInt(gameplayConfig,"bot_decision_max_ms",450,ServerBot::mDecisionMin,60000);
	ServerBot::mPathInterval = LoadServerConfigFloat(gameplayConfig,"bot_path_interval_ms",600.0f,1.0f,60000.0f);
	ServerBot::mWaypointTolerance = LoadServerConfigFloat(gameplayConfig,"bot_waypoint_tolerance",32.0f,1.0f,10000.0f);
	ServerBot::mStopDistance = LoadServerConfigFloat(gameplayConfig,"bot_stop_distance",150.0f,0.0f,10000.0f);
	ServerBot::mKnifeDistance = LoadServerConfigFloat(gameplayConfig,"bot_knife_distance",55.0f,0.0f,10000.0f);
	ServerBot::mGunDistance = LoadServerConfigFloat(gameplayConfig,"bot_gun_distance",700.0f,0.0f,10000.0f);
	char* infectionDelay = GetConfig(gameplayConfig,"infection_respawn_delay");
	mInfectionRespawnDelay = infectionDelay == NULL ? 1 : abs(atoi(infectionDelay));
	delete[] infectionDelay;
	if (mInfectionRespawnDelay > 4) mInfectionRespawnDelay = 4;
	char* roundFreezeTime = GetConfig(gameplayConfig,"round_freeze_time");
	mRoundFreezeTime = roundFreezeTime == NULL ? 3 : abs(atoi(roundFreezeTime));
	delete[] roundFreezeTime;
	if (mRoundFreezeTime > 10) mRoundFreezeTime = 10;

	char* roundTime = GetConfig("data/config.txt","roundtime");
	if (roundTime != NULL) {
		mRoundTime = abs(atoi(roundTime));
		delete[] roundTime;
		if (mRoundTime < 10) {
			mRoundTime = 10;
		}
	...
	char* buyTime = GetConfig("data/config.txt","buytime");
	if (buyTime != NULL) {
		mBuyTime = abs(atoi(buyTime));
		delete[] buyTime;
	}
	else {
		mBuyTime = 60;
	...
	char* mapTime = GetConfig("data/config.txt","maptime");
	if (mapTime != NULL) {
		mMapTime = abs(atoi(mapTime));
		delete[] mapTime;
		if (mMapTime < 1) mMapTime = 30;
		if (mMapTime > 120) {
			mMapTime = 120;
		}
	}
	...
	char* allTalk = GetConfig("data/config.txt","alltalk");
	if (allTalk != NULL && stricmp(allTalk,"on") == 0) {
		mAllTalk = ON;
	}
	else if (allTalk != NULL && stricmp(allTalk,"off") == 0) {
		mAllTalk = OFF;
	}
	else {
		mAllTalk = ON;
	...
	// Remote admin commands are unauthenticated (admin is decided purely by the
	// client-supplied account name), so they are disabled unless explicitly enabled.
	mAllowRemoteAdmin = false;
	char* allowRemoteAdmin = GetConfig("data/config.txt","allowremoteadmin");
	if (allowRemoteAdmin != NULL) {
		if (stricmp(allowRemoteAdmin,"on") == 0) {
			mAllowRemoteAdmin = true;
		}
		delete[] allowRemoteAdmin;
	}

	char* respawnTime = GetConfig("data/config.txt","respawntime");
	if (respawnTime != NULL) {
		mRespawnTime = abs(atoi(respawnTime));
		delete[] respawnTime;
		if (mRespawnTime > 30) {
			mRespawnTime = 30;
		}
	...
	char* spawnGun = GetConfig("data/config.txt","spawngun");
	if (spawnGun != NULL) {
		mSpawnGunIndex = atoi(spawnGun);
		delete[] spawnGun;
		if (mSpawnGunIndex >= mNumGuns || mSpawnGunIndex < 0) {
			mSpawnGunIndex = -1;
		}
	}
	else {
	...
	char* invincibleTime = GetConfig("data/config.txt","invincibletime");
	if (invincibleTime != NULL) {
		mInvincibleTime = abs(atoi(invincibleTime));
	...
	mRoundTime = 30;*/
	mRoundEndTime = 3;
	if (mBotsEnabled && mBotCount > 0) ResetRound(true);
...
void GameServer::Update(float dt)
{
		...
		if (n > 0) {
			...
			if (recvpacket.ReadInt8() == NETVERSION) {
				...
				if (type >= 64) {
					...
					Connection* pidconn = mUdpManager->GetConnection(playerid);
					// Only trust the disguised-playerid redirect when it arrives from the
					// address already bound to that connection. Without this check, any
					// source could impersonate an existing player by guessing their id.
					// A client whose address legitimately changed re-establishes through
					// CONNECT (which is handled with connection == NULL below).
					if (pidconn != NULL && mUdpManager->CompareIP(from, pidconn->addr)) {
						connection = pidconn;
					}
					else {
						connection = NULL;
	...
	if (mWinner != NONE) {
		...
		float roundEndThreshold = (mGameType == HORDE && mWinner == CT) ? mRoundEndTime : mRoundEndTime*0.33f;
		if (mRoundEndTimer >= roundEndThreshold) {
			if (mGameType == HORDE && mWinner == CT) {
				ResetHordeWave();
			}
			else if (mGameType == FFA || mGameType == CTF) {
				ResetRound(true);
			}
			else {
				if (mGameType == HORDE) mHordeWave = 1;
				ResetRound();
			}
		}
	}

	if (mGameType == HORDE && mRoundState == STARTED && mWinner == NONE) {
		mHordeSurvivalTime += dt/1000.0f*mTimeMultiplier;
	}
	...
	if (mRoundState == FREEZETIME) {
		...
		if (mRoundTimer < 0) {
			...
			if (mWinner == NONE) {
				...
				else if (IsRoundBasedMode(mGameType)) {
					if (mGameType == INFECTION) {
						mWinner = CT;
						mNumCTWins++;
						cout << "Survivors Win\n";
					}
					else if (mGameType == HORDE) {
						mWinner = T;
						mNumTWins++;
						cout << "Horde Wins\n";
					}
					else if (mGameType == EXTERMINATION) {
						if (mNumRemainingCTs > mNumRemainingTs) {
							mWinner = CT;
							mNumCTWins++;
							cout << "Counter-Terrorists Win\n";
						}
						else if (mNumRemainingTs > mNumRemainingCTs) {
							mWinner = T;
							mNumTWins++;
							cout << "Terrorists Win\n";
						}
	...
	for (int i=0; i<mPeople.size(); i++) {
		...
		if (mPeople[i]->mIsFiring) {
			bool fire = true;
			GunObject* currentGun = mPeople[i]->GetCurrentGun();
			if (currentGun != NULL && currentGun->mGun->mFireMode == FIREMODE_SEMI) {
			...
			if (fire) {
				...
				if (mPeople[i]->IsBot() && !bullets.empty()) {
					GunObject* gunobject = mPeople[i]->GetCurrentGun();
					if (gunobject == NULL) {
						for (unsigned int j=0; j<bullets.size(); j++) delete bullets[j];
						continue;
					}
					if (gunobject->mClipAmmo > 0) gunobject->mClipAmmo--;
					for (unsigned int j=0; j<bullets.size(); j++) {
						mBullets.push_back(bullets[j]);
						Packet sendpacket;
						...
						for (unsigned int k=0; k<mUdpManager->mConnections.size(); k++) {
							mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[k]);
						}
					}
				}
				/*if (bullets.size() > 0) {
		...
		else if (mGameType == INFECTION && mWinner == NONE) {
			if (mPeople[i]->mState == DEAD && mPeople[i]->mRespawnTime <= 0.0f && mPeople[i]->mTeam == T) {
				int spawnindex = rand()%mMap->mNumTs;
				RespawnPlayer(mPeople[i],mMap->mTSpawns[spawnindex]->x,mMap->mTSpawns[spawnindex]->y);
			}
		}
...
void GameServer::CheckPlayerCollisions(Person* player)
{
	...
	for (unsigned int i=0; i<mMap->mPickups.size(); i++) {
		...
		float dx = player->mX-pickup.x;
		float dy = player->mY-pickup.y;
		if (dx*dx+dy*dy >= Person::mPickupRadius*Person::mPickupRadius) continue;
		if (pickup.type == PICKUP_AMMO) {
			bool collected = false;
			for (int slot=PRIMARY; slot<=SECONDARY; slot++) {
				GunObject* weapon = player->mGuns[slot];
				...
				SendAmmoUpdate(player,slot);
				collected = true;
			}
			if (!collected) continue;
			pickup.active = false;
			pickup.respawnTimer = pickup.respawnTime;
			SendPickupState(pickup,player->mId);
			continue;
		}
		int maximum = pickup.type == PICKUP_HEALTH ? (player->mTeam == T ? Person::mTMaxHealth : Person::mCTMaxHealth) : (player->mTeam == T ? Person::mTMaxArmor : Person::mCTMaxArmor);
		int current = pickup.type == PICKUP_HEALTH ? player->mHealth : player->mArmor;
		if (current >= maximum) continue;
		current += pickup.amount;
		if (current > maximum) current = maximum;
		if (pickup.type == PICKUP_HEALTH) player->mHealth = current;
		else player->mArmor = current;
		pickup.active = false;
		pickup.respawnTimer = pickup.respawnTime;
		SendPickupState(pickup,player->mId);
	}
	...
	while (packet.Index() < packet.Length()) {
		...
		if (type == SERVERINFO) {
			...
			if (connection == NULL || reconnecting == true) {
				...
				// Cap the number of concurrent (including pending, pre-NEWPLAYER)
				// connections so an attacker can't exhaust memory by spamming CONNECT.
				// Drop silently rather than replying (avoids UDP amplification).
				if ((int)mUdpManager->mConnections.size() >= mNumMaxPlayers * 2 + 8) {
					continue;
				}
		...
		switch (type) {
			case NEWPLAYER: {
				...
				if (datalength > (int)sizeof(player->mIcon)) datalength = sizeof(player->mIcon);
				if (datalength > 0) memcpy(player->mIcon,icon,datalength);
				...
				sendpacket.WriteFloat(mTimeMultiplier);
				sendpacket.WriteInt16(mHordeWave);
				sendpacket.WriteInt16(mHordeWaveDelay);
				sendpacket.WriteFloat(mHordeSurvivalTime);
				mUdpManager->SendReliable(sendpacket,connection,true);
				sendpacket.Clear();
				...
				for (int i=0; i<mNumGuns; i++) {
					...
					sendpacket.WriteInt8(mGuns[i].mFireMode);
					sendpacket.WriteInt8(mGuns[i].mPellets);
					sendpacket.WriteInt8(mGuns[i].mScope);
					sendpacket.WriteInt8(mGuns[i].mBuyCategory);
					sendpacket.WriteInt8(mGuns[i].mBuyTeams);
					sendpacket.WriteChar(mGuns[i].mName);
					sendpacket.WriteInt32(mGuns[i].mMuzzleFlashType);
					mUdpManager->SendReliable(sendpacket,connection,true);
					sendpacket.Clear();
				}
				...
				for (int i=0; i<mPeople.size(); i++) {
					if (mPeople[i]->mId != player->mId) {
						...
						sendpacket.WriteInt16(mPeople[i]->mHealth);
						sendpacket.WriteInt16(mPeople[i]->mArmor);
				...
				for (unsigned int i=0; i<mMap->mPickups.size(); i++) {
					if (!mMap->mPickups[i].active) SendPickupState(mMap->mPickups[i],-1,connection);
				}
				...
				for (int i=0; i<mUdpManager->mConnections.size(); i++) {
					...
					sendpacket.WriteInt16(player->mHealth);
					sendpacket.WriteInt16(player->mArmor);
					sendpacket.WriteChar(player->mName);
					mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[i],true);
					sendpacket.Clear();
			...
			case SWITCHTEAM: {
				...
				if (mGameType == HORDE) team = CT;
				else if (mGameType == INFECTION) {
					if (mRoundState == STARTED && player->mTeam != NONE) team = player->mTeam;
					else team = CT;
				}
			...
			case NEWBULLET: {
				...
				if (exists) break;*/
				if (!round) break;
				if (guntype < 0 || guntype >= mNumGuns) break;
			...
			case NEWSHOTGUNBULLET: {
				...
				int ammo = packet.ReadInt8();
				float time = packet.ReadFloat();
				if (guntype < 0 || guntype >= mNumGuns || mGuns[guntype].mPellets <= 1) break;
			...
			case BUY: {
				...
				if (player == NULL) break;
				if (player->mState == DEAD) break;
				if (mGameType != HORDE && player->mIsInBuyZone == false) {
					sendpacket.WriteInt8(MESSAGE);
					sendpacket.WriteInt8(1);
					mUdpManager->SendReliable(sendpacket,connection);
					sendpacket.Clear();
...
void GameServer::UpdateScores(Person* attacker, Person* victim, Gun* weapon) {
	...
	if (mGameType == FFA) {
		...
		if (victim->mTeam == CT) {
			mNumRemainingCTs--;
			victim->mTeam = T;
			EquipMeleeWeapon(victim);
			mNumCTs--;
			mNumTs++;
			mNumRemainingTs++;
			victim->mRespawnTime = mInfectionRespawnDelay*1000;

			Packet teampacket;
			teampacket.WriteInt8(SWITCHTEAM);
			teampacket.WriteInt8(victim->mId);
			teampacket.WriteInt8(T);
			teampacket.WriteInt8(victim->mType);
			for (unsigned int i=0; i<mUdpManager->mConnections.size(); i++) {
				mUdpManager->SendReliable(teampacket,mUdpManager->mConnections[i],true);
			}
		}
		...
		if (mWinner == NONE && mNumRemainingCTs <= 0) {
			mWinner = T;
			mNumTWins++;
			cout << "Infected Win\n";
			Packet winpacket;
			winpacket.WriteInt8(WINEVENT);
			winpacket.WriteInt8(mWinner);
			for (unsigned int i=0; i<mUdpManager->mConnections.size(); i++) {
				mUdpManager->SendReliable(winpacket,mUdpManager->mConnections[i],true);
			}
			mTimeMultiplier = 0.33f;
			Packet timepacket;
			timepacket.WriteInt8(TIMEMULTIPLIER);
			timepacket.WriteFloat(mTimeMultiplier);
			for (unsigned int i=0; i<mUdpManager->mConnections.size(); i++) {
				mUdpManager->SendReliable(timepacket,mUdpManager->mConnections[i],true);
			}
		}
	}
	else if (IsRoundBasedMode(mGameType)) {
		...
		if (mWinner == NONE && (mNumRemainingTs == 0 || mNumRemainingCTs == 0)) {
			...
			mTimeMultiplier = (mGameType == HORDE && mWinner == CT) ? 1.0f : 0.33f;
			sendpacket.WriteInt8(TIMEMULTIPLIER);
			sendpacket.WriteFloat(mTimeMultiplier);
			for (int i=0; i<mUdpManager->mConnections.size(); i++) {
				mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[i],true);
...
void GameServer::ResetRound(bool fullreset) {
	...
	Packet sendpacket = Packet();
	if (mAutoBalance == ON && mGameType != FFA && mGameType != INFECTION && mGameType != HORDE) {
	...
	for (unsigned int i=0; i<mMap->mPickups.size(); i++) SendPickupState(mMap->mPickups[i]);
	...
	for (unsigned int i=0;i<mPeople.size();i++) {
		...
		if (fullreset) {
			...
			mPeople[i]->mMoney = Person::mStartingMoney;
		}
		...
		if (mGameType == FFA) {
			if (mMap->mNumCTs > 0) {
				x = (int)mMap->mCTSpawns[ctspawnindex]->x;
				y = (int)mMap->mCTSpawns[ctspawnindex]->y;
			...
			if (mPeople[i]->mTeam == CT) {
				...
				if (mMap->mNumTs > 0) {
					x = (int)mMap->mTSpawns[tspawnindex]->x;
					y = (int)mMap->mTSpawns[tspawnindex]->y;
	...
	if (mGameType == HORDE) {
		char waveMessage[64];
		sprintf(waveMessage,"Horde wave %d",mHordeWave);
		sendpacket.WriteInt8(SERVERMESSAGE);
		sendpacket.WriteChar(waveMessage);
		for (unsigned int i=0; i<mUdpManager->mConnections.size(); i++) {
			mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[i],true);
		}
		SendHordeState();
...
void GameServer::ResetHordeWave() {
	...
	mNumRemainingCTs = 0;
	mNumRemainingTs = 0;
	int ctspawnindex = mMap->mNumCTs > 0 ? rand()%mMap->mNumCTs : 0;
	int tspawnindex = mMap->mNumTs > 0 ? rand()%mMap->mNumTs : 0;
	...
	for (unsigned int i=0; i<mPeople.size(); i++) {
		...
		if (person->mTeam == CT) {
			...
			for (int slot=PRIMARY; slot<=GRENADE; slot++) {
				...
				SendAmmoUpdate(person,slot);
			}
			int x = (int)person->mX;
			int y = (int)person->mY;
			if (mHordeRegroupStyle == 1 && mMap->mNumCTs > 0) {
				x = (int)mMap->mCTSpawns[ctspawnindex]->x;
				y = (int)mMap->mCTSpawns[ctspawnindex]->y;
				ctspawnindex = (ctspawnindex+1)%mMap->mNumCTs;
			}
			RespawnPlayer(person,x,y);
			mNumRemainingCTs++;
		}
	...
	char waveMessage[64];
	sprintf(waveMessage,"Horde wave %d starts in %d seconds",mHordeWave,mHordeWaveDelay);
	sendpacket.WriteInt8(SERVERMESSAGE);
	sendpacket.WriteChar(waveMessage);
	for (unsigned int i=0; i<mUdpManager->mConnections.size(); i++) {
		mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[i],true);
...
void GameServer::RespawnPlayer(Person* player, int x, int y) {
	...
	if (isDead) {
		Gun* spawngun = NULL;
		if (!IsRoundBasedMode(mGameType) && mSpawnGunIndex > 0 && mSpawnGunIndex < mNumGuns) {
		...
		if (player->IsBot()) GiveBotLoadout(player);
...
void GameServer::SendNewPlayer(Person* player) {
	Packet sendpacket;
	sendpacket.WriteInt8(NEWPLAYER);
	sendpacket.WriteInt8(player->mId);
	sendpacket.WriteInt16((int)player->mX);
...
void GameServer::SendPickupState(MapPickup& pickup, int playerId, Connection* target) {
	Packet sendpacket;
	sendpacket.WriteInt8(PICKUPSTATE);
	sendpacket.WriteInt16(pickup.id);
	sendpacket.WriteInt8(pickup.active ? 1 : 0);
	sendpacket.WriteInt8(playerId);
	Person* player = GetPerson(playerId);
	sendpacket.WriteInt16(player == NULL ? 0 : player->mHealth);
	sendpacket.WriteInt16(player == NULL ? 0 : player->mArmor);
	if (target != NULL) {
		mUdpManager->SendReliable(sendpacket,target,true);
		return;
	}
	for (unsigned int i=0; i<mUdpManager->mConnections.size(); i++) {
		mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[i],true);
...
void GameServer::SendHordeState() {
	if (mGameType != HORDE) return;
	Packet sendpacket;
	sendpacket.WriteInt8(HORDESTATE);
	sendpacket.WriteInt16(mHordeWave);
	sendpacket.WriteInt8(mRoundState);
	sendpacket.WriteFloat(mRoundTimer);
	sendpacket.WriteFloat(mHordeSurvivalTime);
...
void GameServer::SendAmmoUpdate(Person* player, int slot) {
	if (player == NULL || slot < 0 || slot >= 5 || player->mGuns[slot] == NULL) return;
	Packet sendpacket;
	sendpacket.WriteInt8(AMMOUPDATE);
	sendpacket.WriteInt8(player->mId);
	sendpacket.WriteInt8(slot);
	sendpacket.WriteInt16(player->mGuns[slot]->mClipAmmo);
	sendpacket.WriteInt16(player->mGuns[slot]->mRemainingAmmo);
	for (unsigned int i=0; i<mUdpManager->mConnections.size(); i++) {
		mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[i],true);
	}
}

void GameServer::AddBot(char* name, int team) {
	if (mNumPlayers >= mNumMaxPlayers) return;
	ServerBot* bot = new ServerBot(&mGunObjects,&mBullets,mUdpManager,name,
		&mPeople,mGrid,mAStar,&mGameType,mFlagX,mFlagY);
	int id = GetPlayerId();
	if (id < 0) {
		delete bot;
		return;
	...
	bot->mTeam = team;
	EquipMeleeWeapon(bot);
	bot->mGunIndex = KNIFE;
	bot->mTime = &mTime;
	bot->mId = id;
	bot->SetState(DEAD);
	strcpy(bot->mAccountName,"server_bot");
	memset(bot->mIcon,0,sizeof(bot->mIcon));
	mPeople.push_back(bot);
	mNumPlayers++;
	if (team == CT) mNumCTs++;
	else if (team == T) mNumTs++;
	SendNewPlayer(bot);
}

void GameServer::EquipMeleeWeapon(Person* player) {
	if (player == NULL) return;
	int weaponId = player->mTeam == T ? ZOMBIECLAWS : 0;
	if (weaponId >= mNumGuns || mGuns[weaponId].mId != weaponId) weaponId = 0;
	if (player->mGuns[KNIFE] != NULL) delete player->mGuns[KNIFE];
	player->mGuns[KNIFE] = new GunObject(&mGuns[weaponId],0,0);
}

void GameServer::AddConfiguredBots() {
	if (!mBotsEnabled) return;
	for (int i=0; i<mBotCount; i++) {
		int team = i < mInfectedBotCount ? T : CT;
		std::string botName = mBotNames[i%mBotNames.size()];
		if (i >= (int)mBotNames.size()) {
			char suffix[12];
			sprintf(suffix," %d",i/(int)mBotNames.size()+1);
			if (botName.length()+strlen(suffix) > 31) botName.resize(31-strlen(suffix));
			botName += suffix;
		}
		char name[32];
		strncpy(name,botName.c_str(),31);
		name[31] = '\0';
		AddBot(name,team);
	}
}

void GameServer::ConfigureModeTeams() {
	if (mGameType != INFECTION && mGameType != HORDE) return;
	int infectedBotsAssigned = 0;
	bool infectedHumanAssigned = false;
	mNumCTs = 0;
	mNumTs = 0;
	for (unsigned int i=0; i<mPeople.size(); i++) {
		if (!mPeople[i]->IsBot() && mPeople[i]->mTeam == NONE) continue;
		int team = CT;
		if (mPeople[i]->IsBot() && infectedBotsAssigned < mInfectedBotCount) {
			team = T;
			infectedBotsAssigned++;
		}
		else if (!mPeople[i]->IsBot() && mGameType == INFECTION &&
			mInfectedBotCount == 0 && !infectedHumanAssigned) {
			team = T;
			infectedHumanAssigned = true;
		}
		bool changed = mPeople[i]->mTeam != team;
		mPeople[i]->mTeam = team;
		EquipMeleeWeapon(mPeople[i]);
		if (team == CT) mNumCTs++;
		else mNumTs++;
		if (changed) {
			...
			sendpacket.WriteInt8(team);
			sendpacket.WriteInt8(mPeople[i]->mType);
			for (unsigned int j=0; j<mUdpManager->mConnections.size(); j++) {
				mUdpManager->SendReliable(sendpacket,mUdpManager->mConnections[j],true);
			}
		}
	}
...
void GameServer::GiveBotLoadout(Person* player) {
	if (player == NULL || !player->IsBot() || player->mGuns[PRIMARY] != NULL) return;
	std::vector<Gun*> choices;
	int buyTeam = player->mTeam == T ? BUY_TEAM_T : BUY_TEAM_CT;
	for (int i=0; i<mNumGuns; i++) {
		if (mGuns[i].mType == PRIMARY && mGuns[i].mBuyCategory != BUY_CATEGORY_NONE &&
			(mGuns[i].mBuyTeams & buyTeam) != 0) {
			choices.push_back(&mGuns[i]);
		}
	}
	if (choices.empty()) return;
	Gun* gun = choices[rand()%choices.size()];
...
int GameServer::GetPlayerId() {
	for (int attempt=0; attempt<64; attempt++) {
		mPlayerCounter = (mPlayerCounter+1)%64;
		bool taken = false;
		for (unsigned int i=0; i<mPeople.size(); i++) {
		...
		for (unsigned int i=0; !taken && i<mUdpManager->mConnections.size(); i++) {
			if (mUdpManager->mConnections[i]->playerid == mPlayerCounter) taken = true;
		}
		if (!taken) return mPlayerCounter;
	}
	return -1;
}
...
char* GameServer::GetConfig(const char *location, char searchstr[], int length) {
	...
	if (file == NULL) return NULL;
		...
		char value[1024];
		while (fgets(line,1024,file)) {
			if (sscanf(line,"%31s %*s %1023[^\r\n]",key,value) == 2 && strcmp(searchstr,key) == 0) {
				char* valueptr = new char[length];
...
void GameServer::Buy(Person* player, int index) {
	...
	bool hasMoney = true;
	if (index >= 0) {
		if (index >= mNumGuns || mGuns[index].mId != index || mGuns[index].mBuyCategory == BUY_CATEGORY_NONE) return;
		int buyTeam = player->mTeam == T ? BUY_TEAM_T : BUY_TEAM_CT;
		if ((mGuns[index].mBuyTeams & buyTeam) == 0) return;
	}
	...
	if (grenade->mGrenadeType == FLASH || grenade->mGrenadeType == HE) {
		for (unsigned int i=0; i<mPeople.size(); i++) {
			...
			if (grenade->mGrenadeType == FLASH) {
				...
				if (scope == SCOPE_LOW) a = 800;
				else if (scope == SCOPE_MEDIUM) a = 1100;
				else if (scope == SCOPE_HIGH) a = 1400;
...
void GameServer::HandleInput(char* input, bool remote) {
	...
	if (input[0] == '/') {
		...
		if (stricmp(command,"help") == 0) {
			...
			cout << "  /map [map name] [type] - changes map (tdm, ctf, ffa, extermination, infection or horde)\n";
			...
			if (n == 3) {
				...
				else if (stricmp(value2,"extermination") == 0) {
					type = EXTERMINATION;
				}
				else if (stricmp(value2,"infection") == 0) {
					type = INFECTION;
				}
				else if (stricmp(value2,"horde") == 0) {
					type = HORDE;
				}
			}
...
bool GameServer::LoadMap(char* mapname, int maptype) {
	...
	// Reject map names that would overflow mapfile or escape the maps/ directory.
	if (mapname == NULL) return false;
	int namelen = (int)strlen(mapname);
	// Name must fit both mapfile ("maps/%s/overview.png") and mMapName[32].
	if (namelen == 0 || namelen > (int)sizeof(mMapName) - 1) return false;
	for (int i=0; i<namelen; i++) {
		char c = mapname[i];
		if (c == '/' || c == '\\' || c == ':') return false;
	}
	if (strstr(mapname,"..") != NULL) return false;

	snprintf(mapfile,sizeof(mapfile),"maps/%s/map.txt",mapname);
	...
	snprintf(mapfile,sizeof(mapfile),"maps/%s/tile.png",mapname);
	...
	snprintf(mapfile,sizeof(mapfile),"maps/%s/overview.png",mapname);
	...
	strncpy(mMapName,(char*)mapname,sizeof(mMapName)-1);
	mMapName[sizeof(mMapName)-1] = '\0';
	mGameType = maptype;
