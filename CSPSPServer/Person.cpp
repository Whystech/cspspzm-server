...
int Person::mArmorDamageReduction = 30;
int Person::mCTMaxHealth = 100;
int Person::mTMaxHealth = 150;
int Person::mCTMaxArmor = 100;
int Person::mTMaxArmor = 100;
int Person::mCTSpawnHealth = 100;
int Person::mTSpawnHealth = 150;
int Person::mCTSpawnArmor = 0;
int Person::mTSpawnArmor = 0;
int Person::mStartingMoney = 800;
float Person::mPickupRadius = 16.0f;
float Person::mDamageSpeedMultiplier = 0.1f;
float Person::mAcceleration = 0.0005f;
float Person::mDeceleration = 0.0005f;
...
Person::Person(std::vector<GunObject*>* gunobjects, std::vector<Bullet*>* bullets, UdpManager* udpmanager, char* name)
{
	...
	mBullets = bullets;
	mMoney = mStartingMoney;
	mTeam = NONE;
	mHealth = mCTSpawnHealth;
	mArmor = mCTSpawnArmor;
...
void Person::TakeDamage(int damage) {
	...
	if (mState != DEAD) {
		SetMoveState(NOTMOVING);
		mSpeed *= mDamageSpeedMultiplier;
		if (mArmor > 0) {
			int absorbedDamage = damage*mArmorDamageReduction/100;
		...
		sendpacket.WriteInt8(mId);
		sendpacket.WriteInt16(mHealth);
		sendpacket.WriteInt16(mArmor);
		for (int i=0; i<mUdpManager->mConnections.size(); i++) {
