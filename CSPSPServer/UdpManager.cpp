...
void UdpManager::SendReliable(Packet packet, Connection* connection, bool ordered, bool round)
{
	...
	if (connection != NULL) {
		...
		if (!ordered) {
			...
			connection->packets.push_back(packet);
			// Memory backstop: a healthy client acks and drains this queue. If it
			// grows this large the connection is effectively dead; stop queuing.
			if (connection->reliablePackets.size() < 4096) {
			...
			//connection->packet.WritePacketData(packet);
			connection->packets.push_back(packet);
			// Memory backstop for a dead/non-acking connection (see reliablePackets).
			if (connection->orderedPackets.size() < 4096) {
...
bool UdpManager::HandleSequence(int ackid, Packet &packet, int start, Connection* connection) {
	if (connection != NULL) { 
		...
		if (d > 1) {
			...
			if (!exists) {
				// Bound the out-of-order buffer so a peer can't exhaust memory by
				// flooding packets with distinct future sequence ids.
				if (connection->bufferedPackets.size() >= 256) {
					return false;
				}
